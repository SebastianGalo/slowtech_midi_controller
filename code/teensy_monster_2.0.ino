...
#include <Encoder.h> // Encoder library, download from http://www.pjrc.com/teensy/td_libs_Encoder.html
...
int pushbuttonMode[32] = {
  ...
  1, 1, 1, 1, //4-7
  1, 1, 1, 1, //8-11
  1, 1, 1, 1, //12-15 (NOTE: comma after last row/number for shift function to continue down under)
...
int s0 = 1; //control pin A - only need one
int s1 = 2; //control pin B - only need one
int s2 = 3; //control pin C - only need one
int s3 = 4; //control pin D - only need one

//INHIBITOR PIN FOR MULTUPLEXERS_________________________ 
int INH_pin = 9; 

//MULTIPLEXER ANALOG IN PINS____________________________
//(64 potetiometers in total)
int SIG_pin1 = 15; //analog mux read pin for mux1
int SIG_pin2 = 16; //analog mux read pin for mux2
int SIG_pin3 = 17; //analog mux read pin for mux3
int SIG_pin4 = 20; //analog mux read pin for mux4

//MULTIPLEXER DIGITAL IN SIGNAL PIN FOR BUTTONS________
//(16 buttons in total)
int SIG_pin5 = 6; //digital mux read pin for muxButtons

//SHIFT REGISTER PIN FOR LED OUTPUT___________
int latchPin = 10;
int clockPin = 14;
int dataPin = 11;
...
int analogPinsTeensy[6] = {21,22,23,24,25,26};
...
void setup() {
  ...
  //SHIFT BUTTON - pin config _______________________________________________
  ...
  //DIGITAL IN - pin config _______________________________________________
  ...
  SPI.setMOSI(11);
  //use alternative SPI clock pin on 14 for not conflicting with internal led on pin 13
  SPI.setSCK(14); 
  SPI.begin();
  
  //SHIFT REGISTER SETUP_______________________________________________
...
void loop() {
  ...
  //SHIFT BUTTON loop _______________________________________________
  if (shiftPin != 0) {
  ...
  //DIGITAL IN MUX loop 5 BUTTONS_______________________________________________ 
  for (int i = 0; i < 16; i++) { //loop through 16 mux channels
    //if (i != shiftPin) { //ensure this is not the shift pin, ger 15 i resultat om inte utslashad
 ...
 // //DIGITAL OUT SHIFT REGISTER FOR LEDS_______________________________________________
 for (int i = 0; i < 16; i++) {
    //load the button state from the array
    data = buttonState[i];
    byte j;
